    ...
    stage('Create BuildConfig') {
      ...
      steps {
        script {
          openshift.withCluster() {
            openshift.create(
               '{"apiVersion": "build.openshift.io/v1","kind": "BuildConfig","metadata": {"annotations": {"description": "Defines how to build the application"},"labels": {"app": "forecaster"},"name": "forecaster-buildconfig","namespace": "egis"},"spec": {"failedBuildsHistoryLimit": "5","output": {"to": {"kind": "ImageStreamTag","name": "forecaster:latest"}},"postCommit": {"script": ""}, "runPolicy": "Serial","source": {"git": {"uri": "https://github.com/INTEGRITY-One/egis-forecast.git/forecaster"},"type": "Git"},"strategy": {"type": "Source","sourceStrategy": {"from": {"kind": "ImageStream","namespace": "openshift","name": "java"}}},"successfulBuildsHistoryLimit": "5"}}'
            )
          }
        }
      }
